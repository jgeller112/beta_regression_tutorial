...
<!-- BK: It's generally a good idea to have a `setup` chunk at the top that loads libraries / sets 
document-level parameters. I would also recommend using `renv` to set up a paper-specific 
package repo so that future versions of R packages don't break the code.
I.e. see https://rstudio.github.io/renv/articles/renv.html 
I already had one issue running an older version of marginaleffects with the vignette -->
...
ols_model_new %>%
 tt(digits = 2) %>%
    format_tt(j = "p", fn = scales::label_pvalue(),escape = TRUE)
...
# note: I'm getting a really weird error (the plot is too big)
# when I set panel=FALSE, I dont't get an error but also no output from the command.

check_model(ols_model, check=c("homogeneity", "normality"),panel=FALSE)
```
...
# note: I'm getting the below error on Mac OS X 
...
# note BK: I'm not getting s.value as an output from avg_predictions
...
# again I'm not getting these same unwanted variables
...
avg_comparisons(beta_model, comparison = "lnoravg",
  ...
  tt() %>%
  format_tt(digits=3,escape = TRUE)
