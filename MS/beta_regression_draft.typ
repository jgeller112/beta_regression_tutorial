...
On the surface this may appear as an easy question to answer, but the statistical analysis of proportions can present numerous difficulties that are often not taken into consideration. By definition, proportions are limited to numerical values between, and including, 0 and 1, and the variability in the observed proportions usually varies systematically with the mean of the response. It is quite common, especially in psychology, to analyze proportional outcomes using methods falling under the general linear model (GLM), which include techniques like t-tests and ANOVAs. However, there are several issues with the GLM approach. First, the GLM assumes the residuals in the model are normally distributed. Second, it assumes an unbounded distribution that can extend from -$oo$ to $oo$. Lastly, the GLM assumes constant residuals—homoscedasticity. These assumptions are often violated when dealing with proportional data, which are typically bounded between 0 and 1, may not follow a normal distribution, and are heteroscedastic in nature \[\@ferrari2004; \@paolino2001\]. Adopting a model that does not capture your data accurately can have deleterious consequences, such as missing a true effect when it exists (Type 2 error), or mistaking an effect as real when it is not (Type 1 error). A goal for any researcher trying to draw inferences from their data is to fit a model that accurately captures the important features of the data, and has predictive utility \[\@yarkoni2017\].

The issues related to analyzing proportional data are not new (see (#link(<ref-bartlett1936>)[Bartlett, 1936];)). Luckily, several analysis strategies are available to deal with them. One approach we highlight here is beta regression (#link(<ref-ferrari2004>)[Ferrari & Cribari-Neto, 2004];; #link(<ref-paolino2001>)[Paolino, 2001];) and some of its alternatives. With the combination of open-source programming languages like R (#link(<ref-R>)[R Core Team, 2024];) and the great community of package developers, it is becoming trivial to run analyses like beta regression. However, adoption of these methods, especially in psychology, is sparse. A quick Web of Science search for a 10 year period spanning 2014-2024 using (TS=(Psychology)) AND TS=(beta regression) as search terms returned fewer than 20 articles. One reason for the lack of adaptation could be the lack of resources available to wider community (but see (#link(<ref-bendixen2023>)[Bendixen & Purzycki, 2023];; #link(<ref-heiss2021>)[Heiss, 2021];; #link(<ref-vuorre2019>)[Vuorre, 2019];). We attempt to the rectify this herein.
...
Because frequentist statistics are still quite popular in psychology we highlight how to perform beta regression using frequentist packages like `betareg` (#link(<ref-betareg>)[Cribari-Neto & Zeileis, 2010];) and `glmmTMB` \[Brooks et al. (#link(<ref-glmmTMB>)[2017];); useful if you have nested/multilevel data, or working with more complex models)\]. We also highlight how to run these models within a Bayesian framework using the `brms` (#link(<ref-brms>)[Bürkner, 2017];) package. Our main goal for this tutorial is for it to be maximally useful regardless of statistical proclivities of the user.
...
An important freature of the beta distribution is that it can take on a number of different shapes. The location, skew, and spread of the beta distribution is controlled by two parameters: #emph[shape1] and #emph[shape2];. `Shape 1` is sometimes called $alpha$ and `shape 2` is sometimes called $beta$. Together these two parameters shape the density curve of the distribution. For example, let’s suppose a participant got 4 out of 6 correct on a test item. We can take the number of correct on that particular test item (4) and divide that by the number of correct (4) + number of incorrect (2) and plot the resulting density curve. Shape1 in this example would be 4 (number of points received). This parameter reflects the number of successes. `Shape2` would be 2–the number of points not received. This parameter reflects the number of failures. Looking at #link(<fig-beta-dist>)[Figure~1] (a) we see the distribution for one of our questions is shifted towards one indicating higher accuracy on the exam.~If we reversed the values of the two parameters #link(<fig-beta-dist>)[Figure~1] (b), we would get a distribution shifted towards 0, indicating a lower accuracy. By adjusting the values of two parameters, we can get a wide range of distributions (e.g., u-shaped, inverted u-shaped , normal, or uniform). As can be seen, the beta distribution is a distribution of proportions or probabilities.
...
#par()[#text(size:0.5em)[#h(0.0em)]]
#v(-18pt)
However, the canonical parametrization of $alpha$ and $beta$ does not lend itself to the regression framework. Thus, it is quite common to talk about $mu$ and $phi.alt$ instead, where $mu$ represents the mean or average, and $phi.alt$ represents the precision. We can reparameterize $alpha$ and $beta$ into $mu$ and $phi.alt$:
...
==== OLS regression.
<ols-regression>
We first start by fitting a regression model using the `lm` function to the data looking at final test accuracy (`Accuracy`) as a function of instructor fluency (`fluency_dummy1`). A quick look at #link(<fig-flu1>)[Figure~2] shows us accuracy is higher in the fluent condition vs.~the disfluent condition. Is this difference reliable?
...
#par()[#text(size:0.5em)[#h(0.0em)]]
#v(-18pt)
Focusing on output from our regression analysis in #link(<tbl-ols>)[Table~3] , we see the that there is a significant effect of Fluency (`Fluency_dummy1`), b = -0.084 , SE = 0.042 , 95% CIs = \[-0.168,-0.001\], p = 0.048. This is exactly what Wilford et al. (#link(<ref-wilford2020>)[2020];) found.

== Beta regression approach
<beta-regression-approach>
#par()[#text(size:0.5em)[#h(0.0em)]]
#v(-18pt)
Using a traditional approach, we observed instructor fluency impacts actual learning. Keep in mind the traditional approach assumes normality of residuals and homoscadacity. Does the model meet those assumptions? Using `easystats` (#link(<ref-easystats>)[Lüdecke et al., 2022];) and the `check_model` function, we can easily assess this. In #link(<fig-ols-assump>)[Figure~3] , we see there are some issues with our model. Specifically, there is appears to violations of normality and homoscakdacity.
...
#par()[#text(size:0.5em)[#h(0.0em)]]
#v(-18pt)
Looking at #link(<tbl-01s>)[Table~4];, we have 9 rows with accuracy of 0, and 1 row with an accuracy of exactly 1. To run a beta regression, we can employ a little hack. We can nudge our 0s towards .01 and our 1s to .99 so they fall within the interval of \[0-1\].#footnote[In the newest version of betareg you can model data inclusive 0-1 by including a xdist argumement.]
...
#par()[#text(size:0.5em)[#h(0.0em)]]
#v(-18pt)
Interpreting the output in #link(<tbl-ame1>)[Table~7];, we see the difference between the fluent condition and disfluent condition is 9%. That is, participants who watched a fluent instructor scored 9% higher on the final recall test than participants who saw the disfluent instructor. This difference is reliable, b= -0.095, SE = 0.042, 95 % CIs \[-0.177, -0.013 \], p = 0.023.
...
Beta regression model summary for $phi.alt$ parameter exponentiated
...
We wont always need to include dispersion parameters for each of our variables. We advise conducting a very simple likelihood ratio test (LRT) to examine if we need to include dispersion into our model. To test this we use the `test_likelihoodratio` from the `easystats` ecosystem (#link(<ref-easystats>)[Lüdecke et al., 2022];).
...
We can recreate the beta model from `betareg` in `brms`. Instead of using the `|` operator to specify different parameters like we did in `betareg`, we model each parameter independently. Recall we are fitting two parameters— $mu$ and $phi.alt$. We can easily do this by using the `bf` function from `brms`. `bf()` facilitates the specification of several sub-models within the same formula call. We fit two formulas, one for $mu$ and one for $phi.alt$ and store it in `model_beta_bayes`. Here we allow precision to vary as a function of Fluency (this model fit the data better than a model with an intercept-only dispersion parameter).

We first start by loading in the `brms` (#link(<ref-brms>)[Bürkner, 2017];) and `cmdstanr` (#link(<ref-cmdstanr>)[Gabry et al., 2024];)packages.
...
#par()[#text(size:0.5em)[#h(0.0em)]]
#v(-18pt)
Using the `bf` function, we specify two models—one for $mu$ and one for $phi.alt$. We then pass `model_beta_bayes` to the `brm` function and set the model family to the beta distribution, which is native to our model using the `brm` function. We also set a bunch of arguments to speed up the fitting of the models, which we will not explain herein.
...
#par()[#text(size:0.5em)[#h(0.0em)]]
#v(-18pt)
#link(<tbl-beta-model-zero>)[Table~16] provides a summary of the output for our zib model. As before, the 𝜇 parameter estimates, which have the conditional tag in the Component column are on the logit scale; while 𝜙 parameter coefficients (tagged as dispersion in the Component colum) are on the log scale. In addition, the zero-inflated parameter estimates (tagged as zero-inflated in the Component column) are on the logit scale.
...
#par()[#text(size:0.5em)[#h(0.0em)]]
#v(-18pt)
The estimates provided in #link(<tbl-predict-zero>)[Table~17] are percentages of zeros in the model. We can see there are fewer 0s in the fluent condition (2%) compared to the disfluent condition (15%).
...
#par()[#text(size:0.5em)[#h(0.0em)]]
#v(-18pt)
Luckily, we can fit a ZIB model using `brms` and use the `marginaleffects` package to make inferences about our parameters of interest. Similar to our beta model we fit in `brms`, we will use the `bf()` function to fit several models. We fit our $mu$ and $phi.alt$ parameters as well as our zero-inflated parameter ($alpha$; here labeled as `zi`). In `brms` we can use the zero\_inflated\_beta family argument which is native to `brms`.
...
We can use `marginaleffects` to get get the predicted probability from our model. To get the zero-inflated part the model, we can set `dpar` argument to `zi` in `avg_predictions`. #link(<tbl-brms-zib-predict>)[Table~20] shows predicted probabilities of zero for each level of fluency.
...
#par()[#text(size:0.5em)[#h(0.0em)]]
#v(-18pt)
If we take a look at the summary output in #link(<tbl-ordbeta-glmm>)[Table~23];, we can interpret the values similar to a beta regression, where the conditional effects are on the log odds scale. Here the `Fluency_dummy1` parameter is not statistically significant, #emph[p] = .05.
...
#box(image("beta_regression_draft_files/figure-typst/ppcheck-1.svg"))
...
#block[
#box(image("beta_regression_draft_files/figure-typst/ppcheck-2.svg"))
...
Comparing our analysis with that conducted by \@wilford2020, we demonstrated that using the traditional approach (i.e., #emph[t];-test) to analyze accuracy data can lead to inaccurate inferences. While we did reproduce the results from \@wilford20202, our use of a beta regression model, which accounts for both the mean and precision, revealed no significant effect of fluency. By fitting a zero-inflated beta model, which models the structural 0s in the data, we did find an effect of fluency, but not on the mean component. We found an effect of fluency on the zero-inflated component. of the model. Specifically, those in the disfluent condition were more likely to perform poorly (with a higher probability of 0s). Here, fluency does not affect the mean accuracy, but instead influences the structural zero part of the model. This finding has important theoretical implications and should be included in discussions of fluency effects. Additionally, we demonstrated how to fit data with zeros and ones using the ZOIB and ordered beta models. These analyses Again these analyses highlighted the importance of fitting a model to the data you have.
...
#block[
Gabry, J., Češnovar, R., Johnson, A., & Bronder, S. (2024). #emph[Cmdstanr: R interface to ’CmdStan’];. #link("https://mc-stan.org/cmdstanr/")
