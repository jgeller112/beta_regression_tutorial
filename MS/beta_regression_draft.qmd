...
```{r}
#| label: tbl-columnkey
#| tbl-cap: Columns and values from the dataset
...
We first fit a regression model using the `lm`function to the data looking at final test accuracy (`Accuracy`) as a function of instructor fluency (`fluency_dummy1`). In @fig-flu1, we see that accuracy is higher in the fluent condition vs. the disfluent condition. Is this difference reliable?
...
```{r}
#| label: tbl-01s
#| tbl-cap: Amount of zeros and ones in our dataset
...
Looking at @tbl-01s, we have 9 rows with accuracy of 0, and 1 row with an accuracy of exactly 1. To run a beta regression, we can employ a little hack. We can nudge our 0s towards .01 and 1s to .99 so they fall within the interval of \[0-1\].[^3]
...
No errors this time! Now, Let's interpret the results of our beta regression. @tbl-beta-cond includes the output of our beta regression model for the conditional or $\mu$ parameter
...
#| echo: true
#| tbl-cap: Predicted probablities for fluency
#| tbl-cap-location: top
...
To get predicted probability from our model, we can set `dpar` argument to `zi` in `avg_predictions`. @tbl-brms-zib-predict shows predicted probabilities of 0 for each level of fluency.
