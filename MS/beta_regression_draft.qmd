...
Because frequentist statistics are still quite popular in psychology we show to perform beta regression using frequentist packages like `betareg` [@betareg] and `glmmTMB` [@glmmTMB] (if you have nested type data or working with more complex models). We, however, also highlight how to run these models from a Bayesian framework using the `brms` [@brms] package. Our main goal for this tutorial is be maximally useful regardless of statistical proclivities.
