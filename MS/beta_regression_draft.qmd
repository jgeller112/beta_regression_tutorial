...
plot_predictions(beta_model, condition="Fluency_dummy", what="mu") +
  theme_lucid(base_size=14) + 
...
# get risk difference 
beta_avg_comp<- avg_comparisons(beta_model, what = "mu", newdata=fluency_data) 
...
avg_comparisons(beta_model, what = "mu", newdata=fluency_data,  comparison = "lnoravg",
    transform = "exp") %>%
...
beta_model<-gamlss(Accuracy~Fluency_dummy, sigma.formula = ~1,  data=data_beta, family=BE)
...
beta_model_dis<-gamlss(Accuracy~Fluency_dummy, sigma.formula = ~Fluency_dummy,  data=data_beta, family=BE)
```
...
beta_phi_intercept <- beta_model_dis_output  %>% 
  as.data.frame() %>%
   filter(Component == "sigma", Parameter == "(Intercept)") %>% 
  pull(Coefficient)

# get phi for fluency
beta_phi_fluency <- beta_model_dis_output %>% 
  as.data.frame() %>%
   filter(Component == "sigma", Parameter == "Fluency_dummy1") %>% 
  pull(Coefficient)
