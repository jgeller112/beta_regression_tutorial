...
We first start by fitting a regression model using the `lm` function to the data looking at final test accuracy (`Accuracy`) as a function of instructor fluency (`fluency_dummy1`). A quick look at @fig-flu1 shows us accuracy is higher in the fluent condition vs. the disfluent condition. Is this difference reliable?
...
Focusing on output from our regression analysis in @tbl-ols , we see the that there is a significant effect of Fluency (`Fluency_dummy1`), b = `r round(ols_model_new$Coefficient[2], 3)` , SE = `r round(ols_model_new$SE[2], 3)` , 95% CIs = \[`r round(ols_model_new$CI_low[2], 3)`,`r round(ols_model_new$CI_high[2], 3)`\], p = `r round(ols_model_new$p[2], 3)`. This is exactly what @wilford2020 found.
...
Looking at @tbl-01s, we have 9 rows with accuracy of 0, and 1 row with an accuracy of exactly 1. To run a beta regression, we can employ a little hack. We can nudge our 0s towards .01 and our 1s to .99 so they fall within the interval of \[0-1\].[^3]
...
Interpreting the output in @tbl-ame1, we see the difference between the fluent condition and disfluent condition is 9%. That is, participants who watched a fluent instructor scored 9% higher on the final recall test than participants who saw the disfluent instructor. This difference is reliable, b= `r round(beta_avg_comp$estimate, 3)`, SE = `r round(beta_avg_comp$std.error, 3)`, 95 % CIs \[`r round(beta_avg_comp$conf.low, 3)`, `r round( beta_avg_comp$conf.high, 3)` \], p = `r round(beta_avg_comp$p.value, 3)`.

We can also get the odds ratio (see @tbl-or).
...
We can easily plot the $\mu$ parameter part of model using the `marginaleffects` package and the `plot_predictions` function (see @fig-plot-pre).
...
#| label: tbl-phi-beta
#| echo: true
#| tbl-cap: Beta regression  model summary for fluency factor $\phi$ parameter 
...
We first start by loading in the `brms` [@brms] and `cmdstanr` [@cmdstanr]packages.
...
Parameter estimates from the posterior distribution are presented in @tbl-beta-brms.[^5] To make the output more readable, each model parameter is labeled with a prefix before the variable name, except for the $\mu$ parameter, which takes the same name as the variables in your model. Comparing results with \``` betareg` ``in `model_beta`, our results are very similar. Additionally, the parameters can be interpreted in a similar manner and we can use `marginaleffects` to extract marginal effects and risk difference. [^6]Contrary to frequentist models, there are no *p*-values to interpret in Bayesian models. There is a metric in the table that is included with models fit when using `easystats` and `bayestestr` called probability of direction (pd) that gives an indication of how much of the posterior distribution estimate is in one direction (positive or negative). The pd measure appears to correlated with *p*-values [see @makowski2019; @makowski2019a]. A *pd* of **95%**, **97.5%**, **99.5%** and **99.95%** correspond approximately to two-sided *p*-value of respectively **.1**, **.05**, **.01** and **.001**. Instead of p-values one can look at the 95% credible interval to see if it includes 0--if it does not then the effect can be said to be significant. In the table below the 95% credible intervals are located in the CI_low and CI_high columns. The results are similar to what we found with \``` betareg` ``in `model_beta`.
...
The estimates provided in @tbl-predict-zero are percentages of zeros in the model. We can see there are fewer 0s in the fluent condition (2%) compared to the disfluent condition (15%).
...
Interpreting the estimate in @tbl-marg-zib, seeing lecture videos with a fluent instructor reduces the proportion of zeros by about 13%. Here we have evidence that participants were more likely to do more poorly after watching a disflueny lecture than a fluent lecture.
...
We can use `marginaleffects` to get get the predicted probability from our model. To get the zero-inflated part the model, we can set `dpar` argument to `zi` in `avg_predictions`. @tbl-brms-zib-predict shows predicted probabilities of zero for each level of fluency.
...
In @tbl-marg-zib-brms, we get the risk difference between each level of fluency. With a Bayesian implementation we get 95% credible intervals and we see the difference is significant.
...
We can easily plot the zero-inflated part of the model using the `plot_predictions` function (see @fig-brms-zib)
...
To fit an ordered beta regression in a Bayesian context you can use `ordbetareg` [@ordbetareg] package.
