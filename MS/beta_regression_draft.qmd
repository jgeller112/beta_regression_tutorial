...
ggplot(electors_df, aes(x = electors)) +
  ...
  labs(title = "Density Plot of Electors", x = "Electors", y = "Density") +
  theme_minimal()
...
One solution is to run a beta regression model. Below we fit a beta regression using the `glmmTMB`package [@glmmTMB]. This a popular package for running frequentist beta regressions. Other packages that can be used to run beta regression include `betareg` [@betareg] and also `gamlss` [@gamlss]. In `glmmTMB` we fit a beta regression by using a formula simialr to the lm we fit above. However, we must specify the family argument as `beta_family` to fit a beta regression.
...
beta_model <- glmmTMB(Accuracy~Fluency_dummy, disp=~1, data=fluency_data, family=beta_family(link="logit"))# fits a constant for mu and dispersion
...
beta_model<- glmmTMB(Accuracy~Fluency_dummy, disp=~1, data=data_beta, family=beta_family(link="logit"))# fits a constant for mu and dispersion
...
beta_model<-glmmTMB(Accuracy~Fluency_dummy, disp=~1, data=data_beta, family=beta_family(link="logit"))
...
beta_model_dis<-glmmTMB(Accuracy~Fluency_dummy, disp=~Fluency_dummy, data=data_beta, family=beta_family(link="logit"))
```
...
model_parameters(beta_brms, "mean")%>%
    mutate(Parameter=str_remove_all(Parameter, "_")) %>%   rename(CIlow="CI_low",CIhigh="CI_high") %>% 
  tt() %>%
  format_tt(digits=3)
...
#| 
 beta_model_0 %>%
  ...
  mutate(term=str_remove(term, "_")) %>% 
  rename(PredictedHigh="predicted_hi",
...
fit_zi %>% 
  ...
  mutate(term=str_remove(term, "_")) %>% 
  rename(PredictedLow="predicted_lo",
