...
The plausible range of percentages/probabilities of @fig-trump1 is now within 48% to 52% as taking into account Trump’s 2016 results increases our sample size while also increasing our most likely estimate of his 2024 performance. Importantly, the Beta distribution respects bounds.
...
However, the canonical parametrization of $\alpha$ and $\beta$ does not lend itself to the regression framework because we would need to fit models to both parameters at the sam etime. Thus, it is quite common to talk about a re-parameterization involving two different parameters, $\mu$ and $\phi$, where $\mu$ represents the mean or average, and $\phi$ represents the precision, in a roughly analogous way to how the Normal distribution has a mean and variance. We can reparameterize $\alpha$ and $\beta$ into $\mu$ and $\phi$ via the following algebraic relationship:

<!-- Note: the \begin{equation} doesn't need to used if you also have \begin{aligned} -->
...
# requirements:
# patchwork v1.2.0 or higher
# marginaleffects 0.21.0 or higher
# I also had to do tlmg_install fontaxes
...
@tbl-ols displays the summary of the ols model. It shows two coefficients. The `Intercept` value refers to the accuracy in the fluent condition. The Fluency coefficient (`Fluency_dummy1`) highlights the difference between the fluent and disfluent condition, which is significant, b = `r round(ols_model_new$Coefficient[2], 3)` , SE = `r round(ols_model_new$SE[2], 3)` , 95% CIs = \[`r round(ols_model_new$CI_low[2], 3)`,`r round(ols_model_new$CI_high[2], 3)`\], p = `r round(ols_model_new$p[2], 3)`. This analysis reproduces what @wilford2020 found—better memory for fluent instructor videos.

## Beta regression approach

Using a traditional approach, we observed instructor fluency impacts actual learning. Keep in mind the traditional approach assumes normality of residuals and homoscadacity. These assumptions are tricky to maintain when the continuous response approaches either the upper or lower boundary of the scale. Does the model meet those assumptions? Using `easystats` [@easystats] and the `check_model` function, we can easily assess this. In @fig-ols-assump , we see there are some issues with our model. Specifically, there appears to be violations of normality and homoscakdacity. **\[BOB: PERHAPS WE SHOULD JUST WRITE "CONSTANT VARIANCE" RATHER THAN HOMOSKEDAST-WHATEVER IT IS\].**
...
One solution is to run a beta regression model. Below we fit a beta regression using the `glmmTMB`package [@glmmTMB]. This a popular package for running maximum likelihood (MLE) beta regressions with varying intercepts/random effects. Other packages that can be used to run beta regression include `betareg` [@betareg] and also `gamlss` [@gamlss]. In `glmmTMB` we fit a beta regression by using a formula simialr to the lm we fit above. However, we must specify the family argument as `beta_family` to fit a beta regression.
...
When you run the above model, an error will appear: `Error in eval(family$initialize) : y values must be 0 < y < 1`. If your remember, the beta distribution can only model responses in the interval \[0-1\], but not responses that are exactly 0 or 1. We need make sure there are no zeros and ones in our dataset.
...
@tbl-01s shows we have 9 rows with accuracy of 0, and 1 row with an accuracy of exactly 1. To run a beta regression, a common hack is to nudge our 0s towards .01 and our 1s to .99 so they fall within the interval of \[0-1\].[^4]

[^4]: In the newest version of betareg you can model data inclusive 0-1 by including a xdist argumement.
...
Let's fit the model again with our little .01 and .99 hack. **\[BOB COMMENT: I'M FINE IF YOU DO THIS AS A DEMONSTRATION, BUT RE: MY EDIT ABOVE, I REALLY THINK WE SHOULD RECOMMEND THEY JUST REMOVE 0S AND 1S. THEY CAN ALWAYS FIT A STANDARD LOGIT MODEL TO THOSE VALUES, WHICH WOULD GIVE YOU BOTH ESTIMATES AND WOULD PREVENT ANY TYPE OF BIAS. IT ALSO WOULD SET UP NICELY THE EXTENSIONS (I.E., WHAT IF WE MANAGED TO COMBINE THESE TWO MODELS? ;)\]**
...
Thankfully, there is a package called **`marginaleffects`** [@marginaleffects] that can help us extract the predictions quite easily.[^5] For a more detailed explanation of the package please check out: <https://marginaleffects.com/>. To get the proportions for each of our categorical predictors we can use the function from the package called `avg_predictions`.
...
plot_predictions(beta_model, condition = "Fluency_dummy", vcov = TRUE) +
  theme_lucid(base_size = 14) +
  scale_x_discrete(
    breaks = c("0", "1"),
...
**\[BOB COMMENT: See below—to get the log odds, you're exponentiating. If you want to talk about log-odds earlier, you should exponentiate those coefs too.\]**
...
We wont always need to include dispersion parameters for each of our variables. We advise conducting a very simple likelihood ratio test (LRT) to examine if we need to include dispersion into our model. **\[BOB COMMENTS: I WOULD PROBABLY WALK A BIT FROM SAYING THEY HAVE TO DO THIS. MAYBE SAY "ONE THING TO CONSIDER IS WHETHER TO MAKE DISPERSION MORE FLEXIBLE. A LR TEST COULD GIVE YOU SOME EVIDENCE ABOUT WHETHER THAT MIGHT BE A WISE MOVE."\]** To test this we use the `test_likelihoodratio` from the `easystats` ecosystem [@easystats].
...
::: callout-note
Should we include loo model comparisons?

bob: that would be the analogue to the LR test, so makes sense
:::

We can also fit beta regression models within a Bayesian framework. Adopting a Bayesian framework often provides more flexibility and allows us to quantity uncertainty around our estimates which makes it more powerful than the frequentist alternative. For the purposes of this tutorial, we will not be getting into the minutiae of Bayesian data analysis (i.e., setting informative priors, MCMC sampling, etc,). For a more in-depth look into Bayesian data analysis I refer the reader to XXX. **\[BOB COMMENT: I always refer people to McElreath\].**

For the following analyses we will be using default priors provided by `brms`. This will get us something tantamount to a frequentist maximum likelihood analysis most of the readers are used to seeing.
...
model_parameters(beta_brms, "mean") %>%
  mutate(Parameter = str_remove_all(Parameter, "_")) %>%
  rename(CIlow = "CI_low", CIhigh = "CI_high") %>%
  tt() %>%
  format_tt(digits = 3)
```
...
model_zi %>%
  ...
  rename(CIlow = "CI_low", CIhigh = "CI_high", dfError = "df_error") %>%
  mutate(
    Parameter = str_remove(Parameter, "_"),
...
beta_model_0 %>%
  ...
  mutate(term = str_remove(term, "_")) %>%
  rename(
    PredictedHigh = "predicted_hi",
    PredictedLow = "predicted_lo"
  ) %>%
  tt()
```
...
### Alternative package for modeling zero-inflated models: `gamlss`
...
model_parameters(fit_zi, "mean") %>%
  ...
  mutate(
    ...
    Component = str_remove_all(Component, "_")
  ) %>%
  tt() %>%
  format_tt(digits = 3)
```
...
The ZIB model works well if you have 0s in your data, but not 1s.[^8] Sometimes it is theoretically useful to model both zeros and ones as separate processes or to consider these values as essentially similar parts of the continuous response, as we show later in the ordered beta regression model. For example, this is important in visual analog scale data where there might be a prevalence of responses at the bounds [@vuorre2019], in JOL tasks [@wilford2020], or in a free-list task where individuals provide open responses to some question or topic which are then recoded to fall between 0-1 [@bendixen2023]. Here 0 and 1 are meaningful; 0 means item was not listed and 1 means item was listed first.

[^8]: In cases where there are large amounts of 1s but no zeros you can fit a one-inflated model. Currently you can do so with the `gamlss` package in R
...
The one difference between the maximum likelihood implementation and the Bayesian implementation we describe below is that the MLE fit requires that the user have 0s and 1s in the data as otherwise the cutpoints cannot be estimated without restrictions. It is possible to impose these restrictions on the optimizer but is beyond the scope of this tutorial. The Bayesian implementation does not have this problem and for this reason it is possible to fit a Bayesian ordered beta regression to data with or without observations at the bounds (such as a response with only values between 0 and 1), which would be particularly useful in a repeated sampling situation in which 0s and 1s could be observed in future datasets even if they are not in this dataset.

#### Predicted probabilities and marginal effects

Remember these values are on the logit scale so we can take the inverse and get predicted probabilities like we have done before. These values are shown in @tbl-ordbeta-pred.
...
**\[BOB COMMENT: the one issue here is that the `model_parameters` function is not returning the cutpoints. Not sure why as it is returning phi. We can access those with the default `summary` function or another function from `posterior` like `summarize_draws`\]:**

The model cutpoints are not reported by default, but we can access them with the R package `posterior`:
...
## Hierarchical of sins

"I don't want to change, what is the least bad option here?"
