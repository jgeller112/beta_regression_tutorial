...
fluency_data <- read.csv(here::here("data", "miko_data.csv")) %>%
  rename(
...
fluency_data <- read_csv(here::here("data", "fluency_data.csv"))
```
...
bayes_reg_model <- brm(
  ...
  data = fluency_data,
  family = gaussian(),
  file = here::here("models", "model_reg_bayes")
)
```
...
beta_brms <- brm(
  ...
  data = data_beta,
  family = Beta(),
  file = here::here("models", "model_beta_bayes_reg_01")
)
```
...
beta_brms_dis <- brm(
  ...
  data = data_beta,
  family = Beta(),
  file = here::here("models", "model_beta_bayes_dis_run01")
)
```
...
fit_zi <- brm(
  formula = zib_model,
  data = data_beta_0,
  file = here::here("models", "bayes_zib_model0not1.rds")
)
```
...
fit_zoib <- brm(
  formula = zoib_model,
  data = fluency_data,
  file = here::here("models", "bayes_zoib_model")
)
```
...
ord_fit_brms <- ordbetareg(
  Accuracy ~ Fluency,
  data = fluency_data,
  file = here::here("models", "bayes_ordbeta_model")
)
```
...
m.phi <- ordbetareg(
  ...
  data = fluency_data,
  phi_reg = 'both',
  file = here::here("models", "bayes_ordbeta_phi_model")
)
```
