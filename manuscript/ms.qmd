...
**As a result, both estimation and inference differ from standard frequentist approaches. To estimate models, the {brms} package relies on [Stan]{.underline}, which uses Markov chain Monte Carlo (MCMC) algorithms (by default it uses the No U )to draw samples from the posterior distribution—that is, the distribution of the model parameters given the observed data and prior information. By default, {brms} runs four chains with 2,000 iterations each. The first 1,000 iterations per chain are used for warmup (or adaptation) and are discarded. The remaining 1,000 iterations per chain are retained as posterior draws, yielding 4,000 total post-warmup samples across all chains.**
