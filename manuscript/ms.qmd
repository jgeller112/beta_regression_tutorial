...
```{r}
#| label: fig-flu1
#| fig-cap: "Dot plot depicting accuracy distributions for the Fluent and Disfluent conditions. Each condition shows individual data points and summary statistics (mean (diamond))"
#|
fluency_data |>
...
**Importantly, pd and BFs do not indicate whether an effect is meaningfully different--it only reflects the proportion of the posterior in one direction. To address questions of practical significance, Readers can also consider the Region of Practical Equivalence (ROPE) with the Cr.Is [@kruschke2015doing]. Unlike a hypothesis test of a point null (e.g., exactly zero), the ROPE defines a range of values that are deemed too small to be of substantive importance. As a rule of thumb [see @kruschke2018], if more than 95% of the posterior distribution lies inside the ROPE, the effect can be considered practically equivalent to that negligible range. If less than 5% lies inside, the effect can be considered meaningfully different. Intermediate cases are typically labeled undecided.
...
As shown in @tbl-marg-phi-brms, **evidence for an effect of Fluency on disperation is inconclusive -- the 95% Cr.I is wide, encompaing negative, nil, and positive values, suggesting uncertainty about the direction and magnitude of the effect.** 
...
In @tbl-ordbeta the credible interval is close enough to zero relative to its uncertainty that we can conclude there likely aren’t differences between the conditions after taking dispersion and the 0s and 1s in our data into account.
