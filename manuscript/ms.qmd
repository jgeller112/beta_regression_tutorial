...
The ROPE can also be obtained using the {marginaleffects} package by specifying the `equivalence` bounds in `avg_comparisons()`. Running the code in @lst-merope returns a ROPE probability of 23%, indicating that there is insufficient evidence to conclude that the effect exceeds 4% or is practically equivalent.
...
@lst-ordbetadispjol **shows the code for fitting an ordered beta regression model for JOLs as a fluency on** $\mu$ **and** $\phi$**. Using {marginaleffects} @lst-ordbetamargjol we see the difference between the conditions is 12% and the probability of direction is higher (pd = 1) suggesting the difference is positive and above zero @tbl-jol.**
...
The zero-inflated beta (ZIB) model accommodates exact zeros but not ones, so we drop only the ones from the data.
...
@tbl-zib-params reports the full ZIB model summary. Conditional ($\mu$) and zero-inflated ($\alpha$) coefficients are on the logit scale; the dispersion ($\phi$) coefficient is on the log scale.
...
The ZIB model has three components, so we extract predictions and marginal effects for $\mu$ and $\alpha$. @lst-zib-preds shows how to obtain predicted probabilities for the conditional mean ($\mu$) and the zero-inflation component (`type = "zprob"`); results appear in @tbl-zib-preds-mu and @tbl-zib-preds-zi.
