...
Many important outcomes in psychological research can be expressed as proportions or percentages—for example, the proportion of correct responses on a test [e.g., @kornell2008], the proportion of looks directed at a particular stimulus in an eye-tracking task [e.g., @james2025], or the percentage of agreement with a given statement or belief [e.g., @Costello2024]. In educational and cognitive research, one common way to assess learning is by measuring the proportion of correct responses on a test.

Consider a memory experiment in which participants read a short passage, engage in a brief distractor task, and then complete a final memory test with 10 short-answer questions. Each question may have a different point value (e.g., question 1 is worth 4 points, question 2 is worth 1 point). A meaningful outcome measure could be the proportion of points earned on each question relative to its total possible value.
...
[^1]: Specifically, for any continous bounded variable $x$, we can rescale this variable to lie within 0 and 1 by using the formula $x' = \frac{x - \min(x)}{\max(x) - \min(x)}$ where $0 \leq x' \leq 1$.
...
By default, the Bayesian models run 2,000 posterior draws,[^4] which allow us to compute quantities such as the posterior mean (similar to a frequentist point estimate) and the 95% credible interval (Cr.I), which is often compared to a confidence interval. In addition, we report the *probability of direction* (pd), which reflects the probability that a parameter is strictly positive or negative. When a uniform prior is used (all values equally likely in the prior), a pd of 95%, 97.5%, 99.5%, and 99.95% corresponds approximately to a point estimate with two-sided *p*-values of 0.10, 0.05, 0.01, and 0.001, respectively (approximately $1 - \frac{p}{2}$). For directional hypotheses, the pd can be interpreted as roughly equivalent to one minus the *p*-value [@marsmanThreeInsightsBayesian2016].

[^4]: The Stan team recommends 2,000 draws as a default, so this is a conservative standard. We note that Stan can converge in as few as 500 draws for less complex models, which may be an easier standard when engaging in model-building.

<!--# (CP) I added a note that this is ONLY true with improper uniform priors. I think this might be what you're referring to by saying it's "approximately" but people are persnickity. You can take or leave this figure, Claude helped me make the plot, but this is what helped it click in my head.-->

```{r}
#| label: fig-p-value-pd-comp
#| fig-cap: "A Bayesian posterior distribution (assuming uniform prior) centered around the point estimate 1.645 and a Frequentist sampling distribution (under the null; centered at 0). In the Bayesian posterior distribution, the blue area represents the $\alpha$ pd, and the red area represents $1-\alpha$ of the distribution. In the Frequentist Sampling distribution the red tail areas represent the rejection region at $\alpha = 0.1$. Note that the mean estimate for the Bayesian posterior distribution falls exactly at the $1- \frac{\alpha}{2}$ cutoff in the Frequentist sampling distribution. In this sense, the pd is equivalent to the one-sided p-value.  "
#| fig-asp: 0.52
...
# Function to find the non-centrality parameter
find_ncp <- function(df, target_percentile = 0.025, target_value = 0) {
  # Use optimization to find ncp
  objective <- function(ncp) {
    (qt(target_percentile, df, ncp) - target_value)^2
  }
  
  result <- optimize(objective, interval = c(-10, 10))
  return(result$minimum)
}

dt_30_ncp <- find_ncp(30, target_percentile = 0.05)

# Define critical values (e.g., for alpha = 0.05, two-tailed)
alpha <- 0.10

upper_critical <- qt(1-alpha/2, df = 30, ncp = 0)
lower_critical <- qt(alpha/2, df = 30, ncp = 0)

# Create the plot
p <- ggplot(data.frame(x = c(-2, 6)), aes(x = x)) +
  # Plot the normal distribution curve
  stat_function(fun = dt, args = list(df = 30, ncp = dt_30_ncp), 
                color = "black", size = 1) +
  
  # Shade the left tail (p-value region)
  stat_function(fun = dt, args = list(df = 30, ncp = dt_30_ncp),
                xlim = c(0, 6),
  ...
  geom_vline(xintercept =dt_30_ncp , color = "red", 
  ...
  labs(
    ...
    subtitle = paste("pd =", (1-alpha/2)*100, "%"),
    x = "",
    y = ""
  ) +
...
# Create the plot
pd <- ggplot(data.frame(x = c(-2, 6)), aes(x = x)) +
  # Plot the normal distribution curve
  stat_function(fun = dt, args = list(df = 30, ncp = 0), 
                color = "black", size = 1) +
  
  # Shade the left tail (p-value region)
  stat_function(fun = dt, args = list(df = 30, ncp = 0),
                xlim = c(-4, lower_critical),
                geom = "area", fill = "red", alpha = 0.3) +
  
  # Shade the right tail (p-value region)
  stat_function(fun = dt, args = list(df = 30, ncp = 0),
                xlim = c(upper_critical, 6),
                geom = "area", fill = "red", alpha = 0.3) +
  
  # Add vertical lines at critical values
  geom_vline(xintercept = upper_critical, color = "red", 
             linetype = "solid", linewidth = 1) +
    geom_vline(xintercept = 0, color = "black", 
             linetype = "solid", linewidth = 1) +
  ...
  # Add labels and formatting
  labs(
    title = "Frequentist Sampling Distribution",
...
As shown in @tbl-marg-phi-brms, the posterior estimates suggest a credible effect of Fluency on dispersion ($\phi$), with disfluent responses showing greater variability. The 95% Cr.I for the fluency contrast does not include zero, indicating meaningful differences in precision.
...
We can harness the power of `marginaleffects` again and plot the posterior difference between the fluent and disfluent conditions (see @fig-zi-plot). In @fig-zi-plot, there is evidence that watching a lecture video with a fluent instructor reduces the probability of a zero response by approximately 13%. The 95% Cr.I for this effect does not include zero, suggesting a meaningful reduction in the likelihood of zero outcomes under fluent instruction. We can harness the power of `marginaleffects` again and plot the posterior probablity of each level (see @fig-zi-plot).
