...
While we do not have time to delve fully into its derivation, the beta distribution is useful for modeling bounded continuous scales because it is the distribution for the probability of an event**. Given that a probability can take on any value from 0 (the event will not occur with certainty) to 1 (the event will occur with certainty)**, the beta distribution can likewise take on virtually any value in that bounded interval. As a consequence, the beta distribution is the maximum entropy distribution for *any* bounded continuous random variable, which means that the beta distribution can represent the full range of possibilities of such a scale.[^4] As a consequence, if we have a continuous scale with upper and lower bounds--and no other special conditions--the beta distribution will in principle provide a very good approximation of the uncertainty of the scale.

[^4]: Technically, this maximum entropy condition is satisfied because the beta(1,1) distribution is uniform over its support. In addition, we assume that the scale has been re-scaled to the $[0,1]$ interval as we describe above.
...
**As a result, both estimation and inference differ from standard frequentist approaches. To estimate models, the {brms} package relies on [Stan]{.underline}, which uses Markov chain Monte Carlo (MCMC) algorithms (by default it uses the No U-Turn Sampler (NUTS )to draw samples from the posterior distribution—that is, the distribution of the model parameters given the observed data and prior information. By default, {brms} runs four chains with 2,000 iterations each. The first 1,000 iterations per chain are used for warmup (or adaptation) and are discarded. The remaining 1,000 iterations per chain are retained as posterior draws, yielding 4,000 total post-warmup samples across all chains.**
...
**In addition, with the fitting of Bayesian models we were able to move beyond the simple dichotomous thinking involved in frequentist statistics (yes there is an effect or no there is not an effect). With the inclusion of pd, ROPE, and when appropriate Bayes Factors, we discovered that while there may be an effect of fluency, the evidence is inconclusive. There is uncertainty both as to whether the effect exists and, if it does, whether the difference is meaningful enough compared to the the original study it is based on exceed our ROPE boundary of +-4%.**

In this tutorial, we demonstrated how to analyze these models from a Bayesian perspective. While we recognize that not everyone identifies as a Bayesian, implementing these models using a Bayesian framework is relatively straightforward--it requires only a single package, lowering the barrier to entry. For those who prefer frequentist analyses, several R packages are available. For example, the {betareg} package [@betareg] {glmmTMB} [@glmmTMB] and {gamlss} [@gamlss] are nice options. To this end, **we** have included supplemental materials demonstrating how to use frequentist packages to analyze the data presented herein.
