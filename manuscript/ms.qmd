...
#| tbl-align: "center"
#| apa-note: "Link functions:  b_mean = logit; b_phi = logit; b_zoi (zero-one inflation) = logit; b_coi (conditional one-inflation) = logit."
...
cleaned_tables <- imap(model_list, function(mod, name) {
  model_parameters(mod, centrality = "mean") %>%
    ...
    mutate(
      ...
      CI = str_glue("[{round(CI_low, 3)}, {round(CI_high, 3)}]"),
      pd = sprintf("%.3f", pd)
    ) %>%
    pivot_longer(cols = c(Mean, CI, pd), names_to = "Stat", values_to = "value") %>%
    mutate(Model = name) %>%
...
pretty_table <- bind_rows(cleaned_tables) %>%
  ...
  mutate(
    Stat = factor(Stat, levels = c("Mean", "CI", "pd")), 
     Parameter = factor(Parameter, levels = c("b_Intercept", "b_FluencyFluent", "b_phi_Intercept", "b_phi_FluencyFluent", "b_zi_Intercept", "b_zi_FluencyFluent",  "b_zoi_Intercept", "b_zoi_FluencyFluent", "b_coi_Intercept", "b_coi_FluencyFluent")
  )
  ) %>%
...
pretty_table |>
  ...
  style_tt(i = c(2:3,5:6, 8:9, 11:12, 14:15, 17:18, 20:21, 23:24, 26:27, 29:30), italic=TRUE, fontsize=.6)  
```
