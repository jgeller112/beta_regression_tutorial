...
When we first fit the model we can fit a model that does not allow dispersion to vary as a function of fluency.
...
However, if we want dispersion to vary as a fucntion of fluency we can easily do that.
...
Note the addition of the `phi_reg` argument in `m.phi`. This argument allows us to include a model that explicitly models the dispersion parameter. Because I am modeling $\phi$ as a function of fluency, I set the argument to `both`.

## Model Parameters

### Mean ($\mu$) and Dispersion ($\phi$)

@tbl-one presents the overall model summary (under `Ordered Beta`). We can use `marginaleffects` again to examine differences between the conditions on each paramter of interest.
...
The credible interval is close enough to zero relative to its uncertainty that we can conclude there likely aren’t substantial differences in dispersion or mean accuracy between the fluent and disfluent conditions.
