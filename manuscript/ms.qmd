...
While it is advisable to model precision, if there is uncertainty about the best model, a relatively agnostic approach would be to compare models, for example with leave one out (loo) cross validation, to examine if a dispersion parameter should be considered in our model.[^6]

[^6]: The model fit statistic LOO-CV can be compared for any set of fitted brms models with the function `loo()`.
...
@lst-ordbetadispjol **shows the code for fitting an ordered beta regression model for JOLs as a fluency on** $\mu$ **and** $\phi$**. Using {marginaleffects} @lst-ordbetamargjol we see the difference between the conditions is 12% and the probability of direction is high (pd = 1) suggesting the difference is positive and above zero @tbl-jol.**
...
**In addition, fitting Bayesian models allowed us to move beyond the dichotomous thinking often associated with frequentist inference (i.e., whether an effect is present or absent). By incorporating indices such as the probability of direction (pd), regions of practical equivalence (ROPE), and, when appropriate, Bayes factors, we found that although there may be an effect of fluency, the evidence is inconclusive. Specifically, there is uncertainty both about whether the effect exists and, if it does, whether it is meaningful in magnitude—particularly relative to the original study, where our ROPE was defined as ±4%.**
