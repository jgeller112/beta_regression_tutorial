...
#| fig-cap: The plots show 100 posterior predicted distributions with the label yrep (light blue), the distribution of accuracy as function of fluency in dark blue for regular regression (A) , beta regression (B), ZIB (C) and ZOIB (D).
#| fig-height: 20
#| fig-width: 8
...
pp_check2 <- pp_check(
  ...
  theme_publication() +
  xlim(0, 1) +
  coord_cartesian(ylim = c(0, 6)) # example limit – adjust as needed
...
XBX regression [@kosmidis2025] is a recently developed approach that also handles boundary values (0s and 1s) natively, and is fit with the `betareg` package. The mean and precision submodels are specified jointly using the `|` operator in the formula.
