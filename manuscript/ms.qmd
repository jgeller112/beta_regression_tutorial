...
**In addition, with the fitting of Bayesian models we were able to move beyond the simple dichotomous thinking involved in frequentist statistics (yes there is an effect or no there is not an effect). With the inclusion of pd, ROPE, and when appropriate Bayes Factors, we discovered that while there may be an effect of fluency, the evidence is inconclusive. There is uncertainty both as to whether the effect exists and, if it does, whether the difference is meaningnful enough compared to the the original study it is based on exceed our ROPE boundary of 4%.**
