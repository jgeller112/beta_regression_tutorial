...
This tutorial aims to address that gap by offering a more comprehensive overview of Beta regression and its extensions, along with practical tools and code to help psychologists apply these methods in their own work. While previous tutorials have largely focused on Beta regression, or a few of its alternatives, we seek to go over several extensions of the Beta model that should cover all applications and interpreting model summaries, we emphasize the importance of interpreting estimates on the response scale.
