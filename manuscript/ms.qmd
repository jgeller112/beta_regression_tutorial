...
Above we extracted the ROPE value using the {bayestestR} package. We can also extract the ROPE using the {marginaleffects} package by setting the equivalaence bounds argument in the `avg_comparisons()` function (@lst-margeffectrope). Running the code below returns ROPE of 23% which means there is not enough evidence to conclude the effect is larger than 2%. 
...
combined_plot <- (pp_check1 / pp_check2 / pp_check3 / pp_check4) +
  plot_annotation(
    tag_levels = 'A',
    tag_prefix = "(",
    tag_suffix = ")"
