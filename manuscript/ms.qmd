...
```{r}
#| label: fig-flu1
#| fig-cap: "Raincloud plot depciting accuracy for Fluent and Disfluent conditions"
...
We fit the model using the `brm()` function from the `brms` package (@lst-loadbrms). The output shows each parameter's posterior summary: The posterior distribution's mean and standard deviation (analogous to the frequentist standard error) and its 95% credible interval (CI), which indicate the 95% of the most credible parameter values. Additionally, the output indicates numerical estimates of the sampling algorithm's performance: Rhat should be close to one, and the ESS (effective sample size) metrics should be as large as possible given the number of iterations specified (default is 4000). Generally, ESS >= 1000 is reccomended [@gelman2013bayesian]. 

When you run the model in @lst-gauss, the following output will appear in your console. This output summarizes the results of the Bayesian regression model. Going forward, results will be reported in table format (@tbl-all). The `Intercept` refers to the mean accuracy in the *disfluent* condition, as fluency was dummy-coded. The fluency coefficient (`FluencyFluent`) reflects the mean difference in recall accuracy between the fluent and disfluent conditions: *b* = `r round(brms_model_new$Mean[2], 3)`. The 95% credible interval for this estimate spans from `r round(brms_model_new$CI_low[2], 3)` to `r round(brms_model_new$CI_high[2], 3)`. These values are shown in the "95% CI" columns of the output. These results closely mirror the findings reported by @wilford2020: *t*(94) = 2.00, *p* = .048, 95% CI [0.06, 16.56].

The table also includes the ESS and Rhat values, both of which are within acceptable ranges, indicating good model convergence. Throughout the tutorial, we focus on the posterior mean estimates and their 95% credible intervals. In addition, we also include pd in the tables, which are provided by the `easystats` `bayestestr` package [@makowski2019a' @makowski2019] and provided a parraelle to *p*-values most readers are probably familar with. 

## Beta Regression

@wilford2020 observed that instructor fluency impacts actual learning, using a t-test. But recall this approach assumes normality of residuals and homoscedacity. These assumptions are unrealistic when the response values approach the scale boundaries [@sladekova2024unicorns]. Does the data we have meet those assumptions? In @fig-reg-assump, we see some issues with our model. Specifically, there appears to be violations of normality constant variance (homogeneity).
...
We can also examine how well the data fits the model by performing a posterior predictive check using the `pp_check()` function from `brms` (@lst-check1). A postertior predictive check involves looking at multiple draws or repetitions from the posterior distribution and plotting it against the observed data. Ideally, the predictive draws (the light blue lines) should show reasonable resemblance with the observed data (dark blue line). In our example (see @fig-ppcheckall (A)) the model-predicted density is slightly too peaked and narrow compared to the data. In addition, some of the draws extend into negative accuracy values.
...
#| lst-label: lst-beta
#| lst-cap: Setting up and fitting a beta model with brm().
#| results: hide
#| eval: false
#| echo: true
...
However, the dataset has nine zeros and one 1. To run a Beta regression, a common hack is to nudge our 0s towards .01 and our 1s to .99 so they fall within the [0, 1] interval. The model from @lst-beta01 uses data (`data_beta`) where 0s and 1s are changed to .01 and .99, respectively.
...
@tbl-all displays the summary from all out Bayesian implementation. The $\mu$ parameter estimates, which are labeled with an underscore `b_`while $\phi$ parameter coefficients are tagged with b`_phi` in the `Parameter` column.
...
We can also get the odds ratio with `avg_comparisons()` (see @lst-ord and @tbl-or).

```{r}
#| label: tbl or
#| lst-label: lst-ord
#| lst-cap: Getting odds ratio for Fluency.
#| echo: true
...
@fig-ppcheckall (B) shows the predictive check for our beta model. The model does a pretty good job at capturing the data, but it could be better.
...
```{r}
#| label: fig-zi-plot
#| fig-cap: "Visualization of predicted difference for zero-inflated part of model"
#| echo: false
...
We then pass the `zoib_model` to our `brm()` function (see @lst-zoib). The summary of the output is in @tbl-all (under `ZOIB`).
...
#| fig-subcap: Light blue lines are model-predicted data and dark blue line is observed data.
#| fig-height: 8
#| fig-width: 8

# Use `plot_spacer()` for the empty 4th spot
combined_plot <- (pp_check1 + pp_check2) /
  (pp_check3 + plot_spacer()) +
  plot_annotation(
    tag_levels = 'A',
    tag_prefix = "(",
    tag_suffix = ")"
