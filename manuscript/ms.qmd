...
brms_model_new <- model_parameters(bayes_reg_model, 
centrality = "mean")
```
...
brms_rope <- bayestestR::rope(bayes_reg_model, 
ci = .95, 
ci_method = "ETI")
```
...
beta_plot <- plot_predictions(beta_brms_dis, 
by = "Fluency")
```
...
# Add a model identifier to each dataset
pred_draws_beta <- avg_predictions(beta_brms_dis, 
variables = "Fluency") |>
  posterior_draws()
```
...
model2 <- read_rds(here("manuscript", "models", "model_beta_bayes_dis_run01.rds"))
